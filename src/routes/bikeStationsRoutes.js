...
var DAO_BikeStations = require('./../DAO/DAO_BikeStations');
...
router.get('/', function (req, res, next) {
    ...
    if (req.query === null) {
        res.status(400).send(response.responseJson(true, 'Wrong queries for bike stations'));
    } else if (req.query != null &&
        (radius === '' || radius === undefined) &&
        (longitude === '' || longitude === undefined) &&
        (latitude === '' || latitude === undefined)) {
        DAO_BikeStations.findAll(function (err, data) {
                if (err) {
